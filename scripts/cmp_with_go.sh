...
#takes any "*.go" file from --> programs_for_cmp <-- directory and runs it with both GOlang and ifj20 compiler, then compares the results
#GOlang compiler is needed as well as compiled ifj20 project
#-s option for running the "slow" compiler
#every directory needs ifj20.go file
...
SLOW_COMPILER="./compiler_slow"
COMPILER_PATH_NAME="./compiler"

for TEST_FILES_PATH in scripts/programs_for_cmp/* ; do

	echo -e "==========${BLUE}${BOLD} TESTING: $TEST_FILES_PATH ${NORMAL}=========="

	for file in "$TEST_FILES_PATH"/* ; do
		if [[ "$file" != ""$TEST_FILES_PATH"/ifj20.go" ]] && [[ "$file" != *.txt ]]; then
			
			if [[ "$1" == "-s" ]]; then	
				compiler_output=$(valgrind --tool=memcheck --leak-check=full --show-reachable=yes --errors-for-leak-kinds=all --error-exitcode=69 "$SLOW_COMPILER" <"$file" 2> /dev/null)
			else	
...
#echo "=========== python-script tests ==========="a
#python2 .scripts/ifj-testsuite/testsuite.py -c ./../../compiler -e "BOOLTHEN,BASE,FUNEXP,MULTIVAL,UNARY"
#echo "=========== END ============"
